...
-Foi o próprio navegador, ao interpretar o HTML da página.

Ao carregar o documento, o navegador encontra referências a outros recursos — como arquivos CSS, JavaScript, imagens e fontes — e automaticamente faz requisições HTTP para buscá-los. Portanto, ninguém precisou clicar neles.


2. Há requisições para endereços de OUTROS domínios? Anote um deles e arrisque um palpite sobre o que seja.

-Sim exite alguns dominios dentre deles deles : 
Google Analytics → mede visitas e comportamento;
CDN → entrega arquivos como fontes, bibliotecas JS ou imagens;
serviço de anúncios → carrega publicidade ou rastreamento.
...
Imagem: algo como image/png, image/jpeg ou image/webp → indica o formato da imagem.
...
- Está no ar. O fato de ele responder com 404 Not Found mostra que a requisição chegou ao servidor e ele conseguiu enviar uma resposta.

2. O erro foi de quem pediu ou de quem respondeu? Justifique.
- Foi um erro do lado de quem pediu (cliente), no sentido de que a URL/recurso solicitado não foi encontrado. O servidor respondeu corretamente informando isso com o status 404.

3. Se o status fosse 500 em vez do que você anotou, a conclusão seria a mesma? Por quê?
- A conclusão seria diferente. O 500 Internal Server Error indica um problema no servidor ao tentar processar a requisição. Portanto:

404 → servidor funcionando, recurso solicitado não encontrado.
500 → servidor funcionando, mas ocorreu um erro interno ao processar a requisição.
...
- Não. Eles continuam na página, apenas sem a formatação visual. Isso mostra que o conteúdo mora principalmente no HTML; o CSS apenas altera sua apresentação.

2. Descreva em uma frase o papel do CSS, com base apenas no que você acabou de ver.
- O CSS controla a aparência e o layout da página — cores, fontes, espaçamento, colunas e posicionamento.
3. Ainda restou algum comportamento (menu que abre, botão que responde)? De qual das três tecnologias ele vem?
- Se algum menu ainda abre ou algum botão continua respondendo, esse comportamento vem do JavaScript (JS).
